...
# get book by published date with dynamic param
@app.get("/books/published_date/", status_code = status.HTTP_200_OK)
async def read_book_by_published_date(published_date: int = Query(gt=1999)):
    books_to_return = []
...
# Query e Path objects fazem a validação de valores que não são BaseModel do Pydantic
# Uma vez que os métodos foram bem sucedidos, os valores de status code previsto em cada
#função são retornados. Se não forem bem sucedidos, ocorre o raise do error